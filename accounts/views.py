...
def register(request):
    if request.method == 'POST':
        ...
        try:
           newuser = Users.objects.create_user(fullname=fullname,
...
def edit(request, listing_id):
    ...
    try:
        items = Properties(name=name, location=location, ptype=type, status=status, area=area, bed=bed, bath=bath, garage=garage, description=desc, photo=photo, price=price)
        items.save()
